 ...
 *   Filter the long_facts only retaining records where
 *    - element is set
 *    - value is set
 *    - value is valid (not -9999)
 *    - element is TMAX or TAVG or TMIN or PRCP   
 */
facts = 
 ...
 * Join the facts and stations by their station_id.
 */
 ...
 *
 * Group the facts by their year, month, day, state and element
 */
grouped_facts =
    
/*
 * Calculate the avg fact value as the value field. In the end you should end up with a record that
 * looks like
 *     year, month, day, state, element, value
 */
unique_facts =
